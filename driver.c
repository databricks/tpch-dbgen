 ...
 * 32/64 bit changes for overflow handling needed additional changes when ported
 * back to windows
 *
...
void gen_tbl(int tnum, DSS_HUGE start, DSS_HUGE count, long upd_num,
             int *completed);
int pr_drange(int tbl, DSS_HUGE min, DSS_HUGE cnt, long num, int reset);
int set_files(int t, int pload);
int partial(int, int, int *completed);
 ...
 */
int pr_cust(customer_t *c, int mode, int reset);
int pr_line(order_t *o, int mode, int reset);
int pr_order(order_t *o, int mode, int reset);
int pr_part(part_t *p, int mode, int reset);
int pr_psupp(part_t *p, int mode, int reset);
int pr_supp(supplier_t *s, int mode, int reset);
int pr_order_line(order_t *o, int mode, int reset);
int pr_part_psupp(part_t *p, int mode, int reset);
int pr_nation(code_t *c, int mode, int reset);
int pr_region(code_t *c, int mode, int reset);
int pr_reset();
...
tdef tdefs[] = {
    ...
    {"orders.tbl", "orders/lineitem tables", 150000, pr_order_line, sd_order,
     LINE, 0},
    {"part.tbl", "part/partsupplier tables", 200000, pr_part_psupp, sd_part,
     PSUPP, 0},
...
// Hacky way to restore tdefs after dbgen is ran so the next call is 'clean'
// ideally, we remove the global definition and pass it around?
tdef tdefs_backup[] = {
    {"part.tbl", "part table", 200000, pr_part, sd_part, PSUPP, 0},
    {"partsupp.tbl", "partsupplier table", 200000, pr_psupp, sd_psupp, NONE, 0},
    {"supplier.tbl", "suppliers table", 10000, pr_supp, sd_supp, NONE, 0},
...
void gen_tbl(int tnum, DSS_HUGE start, DSS_HUGE count, long upd_num,
             int *completed) {
  printf("\ntnum = %d, start = %d, count = %d, upd_num = %d, completed = %d\n",
         tnum, (int)start, (int)count, (int)upd_num, *completed);
  static order_t o;
  supplier_t supp;
  customer_t cust;
  part_t part;
  ...
  for (i = start; count; count--, i++) {
    ...
    case ORDER_LINE:
      ...
      if (set_seeds == 0)
        tdefs[tnum].loader(&o, upd_num, reset);
      break;
    case SUPP:
      mk_supp(i, &supp);
      if (set_seeds == 0) {
        tdefs[tnum].loader(&supp, upd_num, reset);
      }
      break;
    case CUST:
      mk_cust(i, &cust);
      if (set_seeds == 0)
        tdefs[tnum].loader(&cust, upd_num, reset);
      break;
    case PSUPP:
    case PART:
    case PART_PSUPP:
      mk_part(i, &part);
      if (set_seeds == 0) {
          tdefs[tnum].loader(&part, upd_num, reset);
      }
      break;
    case NATION:
      mk_nation(i, &code);
      if (set_seeds == 0)
        tdefs[tnum].loader(&code, 0, reset);
      break;
    case REGION:
      mk_region(i, &code);
      if (set_seeds == 0)
...
void usage(void) {
  fprintf(stderr, "%s\n%s\n\t%s\n%s %s\n\n",
          "USAGE:", "dbgen [-{vf}][-T {pcsoPSOL}]",
          "[-s <scale>][-C <procs>][-S <step>]",
          "dbgen [-v] [-O m] [-s <scale>]", "[-U <updates>]");
  fprintf(stderr, "Basic Options\n===========================\n");
  fprintf(stderr, "-C <n> -- separate data set into <n> chunks (requires -S, "
                  "default: 1)\n");
  ...
  fprintf(stderr, "-S <n> -- build the <n>th step of the data/update set (used "
                  "with -C or -U)\n");
 ...
 * data
 */
int partial(int tbl, int s, int *completed) {
  DSS_HUGE rowcnt;
  DSS_HUGE extra;

  if (verbose > 0) {
    fprintf(stderr, "\tStarting to load stage %d of %d for %s...", s,
            (int)children, tdefs[tbl].comment);
  }

  set_files(tbl, s);

  printf("\ntbl = %d, scale = %d, children = %d, s = %d\n", tbl, (int)scale,
         (int)children, s);
  rowcnt = set_state(tbl, scale, children, s, &extra);
  printf("\nrowcnt = %d, extra = %d completed = %d after set_state\n",
         (int)rowcnt, (int)extra, (int)*completed);

  if (s == children)
    gen_tbl(tbl, rowcnt * (s - 1) + 1, rowcnt + extra, upd_num, completed);
  else
    gen_tbl(tbl, rowcnt * (s - 1) + 1, rowcnt, upd_num, completed);
...
void process_options(int count, char **vector) {
  ...
  while ((option = getopt(count, vector, "b:C:d:fi:hO:P:qs:S:T:U:v")) != -1)
    switch (option) {
...
void validate_options(void) {
  ...
  if (children != 1) {
    ...
    if (step == -1) {
      fprintf(stderr, "ERROR: -S must be specified when generating data in "
                      "multiple chunks\n");
      exit(-1);
    }
  }
  ...
  if (step != -1) {
    if ((children == 1) && (updates == 0)) {
      fprintf(stderr, "ERROR: -S is only valid when generating data in "
                      "multiple chunks or generating updates\n");
      exit(-1);
    }
  }
 ...
 * line handling
 */
int dbgen(long const *scale_, int const *step_, long const *children_,
          int const *table_) {
  DSS_HUGE i;
  int completed = 0;
  bTableSet = 0;
  ...
  if (table_ == NULL) {
    ...
    table = (1 << *table_);
    // use to be set in process_options, but we don't use that now passing args
    // directly only set when explicit table setting passed
    bTableSet = 1;
  }

  pr_reset();  // hack? Reset use of static FILE descriptors in pr_* functions from any previous run
  ...
  delete_segment = 0;
  verbose = 1; // TODO: back to default 0
  set_seeds = 0;
  scale = scale_ == NULL ? 1 : *scale_;
  flt_scale = 1.0;
  updates = 0;
  step = step_ == NULL ? -1 : *step_;
  ...
  children = children_ == NULL ? 1 : *children_;
  d_path = NULL;
  minrow = 0;
  rowcnt = 0;
  ...
  if (verbose >= 0) {
    fprintf(stderr, "%s Population Generator (Version %d.%d.%d)\n", NAME,
  ...
  if (updates) {
    ...
    while (upd_num < updates) {
      if (verbose > 0)
        fprintf(stderr, "Generating update pair #%d for %s", (int)upd_num + 1,
                tdefs[ORDER_LINE].comment);
      insert_orders_segment = 0;
      insert_lineitem_segment = 0;
      delete_segment = 0;
      minrow = upd_num * rowcnt + 1;
      gen_tbl(ORDER_LINE, minrow, rowcnt, upd_num + 1, &completed);
      if (verbose > 0)
        fprintf(stderr, "done.\n");
      pr_drange(ORDER_LINE, minrow, rowcnt, upd_num + 1, 0);
      upd_num++;
    }

    // On completion, set tdefs back to it's original state for a potential
    // new call. Ideally we don't have global tdefs but pass it around I think.
    memcpy(tdefs, tdefs_backup, sizeof(tdefs));
  ...
  **/
   ...
   * any to be built
   */
  printf("i = %d, table = %d, children = %d, minrow = %d, rowcnt = %d\n",
         (int)i, (int)table, (int)children, (int)minrow, (int)rowcnt);
  for (i = PART; i <= REGION; i++)
    if (table & (1 << i)) {
      if (children > 1 && i < NATION) {
        partial((int)i, step, &completed);
      } else {
        minrow = 1;
        if (i < NATION)
        ...
        gen_tbl((int)i, minrow, rowcnt, upd_num, &completed);
        if (verbose > 0) {
          fprintf(stderr, "done.\n");
        }
      }
