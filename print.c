...
int pr_drange PROTO((int tbl, DSS_HUGE min, DSS_HUGE cnt, long num, int reset));
  ...
  if (updates) {
    if (update > 0) /* updates */
      ...
      if (delete_segments) {
        ...
        sprintf(upath, "%s%cdelete.%d", env_config(PATH_TAG, PATH_DFLT),
                PATH_SEP, -update);
      }
...
int pr_order_line(order_t *o, int mode, int reset) {
  ...
  pr_order(o, mode, reset);
  pr_line(o, mode, reset);
 ...
 */
int pr_part(part_t *part, int mode, int reset) {
 ...
 */
int pr_psupp(part_t *part, int mode, int reset) {
 ...
 */
int pr_part_psupp(part_t *part, int mode, int reset) {
  tdefs[PART].name = tdefs[PART_PSUPP].name;
  pr_part(part, mode, reset);
  pr_psupp(part, mode, reset);

  return (0);
}

int pr_supp(supplier_t *supp, int mode, int reset) {
  static FILE *fp = NULL;
...
// Hacky way to reset the 'static FILE' pointers 
// useful between calls of dbgen
void pr_reset() {
  int reset = 1;
  int mode = 0;  // doesn't matter, reset returns early
  pr_cust(NULL, mode, reset);
  pr_line(NULL, mode, reset);
  pr_order(NULL, mode, reset);
  pr_part(NULL, mode, reset);
  pr_psupp(NULL, mode, reset);
  pr_supp(NULL, mode, reset);
  pr_order_line(NULL, mode, reset);
  pr_part_psupp(NULL, mode, reset);
  pr_nation(NULL, mode, reset);
  pr_region(NULL, mode, reset);

  pr_drange(0, 0, 0, 0, reset);
